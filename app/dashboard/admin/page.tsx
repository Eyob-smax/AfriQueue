import Link from "next/link";
import { redirect } from "next/navigation";
import { getCurrentUserRole } from "@/lib/actions/auth";
import { getAdminOverview, getPendingStaff } from "@/lib/actions/admin";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import { Building2, Users, BarChart3, CalendarCheck, UserPlus, UsersRound } from "lucide-react";
import { Button } from "@/components/ui/button";

export default async function AdminDashboardPage() {
  const user = await getCurrentUserRole();
  if (!user) redirect("/auth/login");
  if (user.role !== "ADMIN" && user.role !== "SUPER_ADMIN") redirect("/dashboard");

  const [overview, pendingStaff] = await Promise.all([
    getAdminOverview(),
    getPendingStaff(),
  ]);
  const pendingCount = pendingStaff.length;

  return (
    <div className="space-y-6 p-8">
      <h1 className="text-2xl font-bold text-[#0d1b1a] dark:text-white">
        Admin Overview
      </h1>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="border-slate-200 dark:border-slate-800">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            <UserPlus className="w-5 h-5 text-primary" />
            <h2 className="font-semibold text-sm">Pending staff</h2>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{pendingCount}</p>
            <Link href="/dashboard/admin/staff-registrations">
              <Button
                variant="link"
                className="p-0 h-auto text-xs text-primary"
              >
                {pendingCount === 0 ? "View registrations" : "Review requests"}
              </Button>
            </Link>
          </CardContent>
        </Card>
        <Card className="border-slate-200 dark:border-slate-800">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            <Building2 className="w-5 h-5 text-primary" />
            <h2 className="font-semibold text-sm">Health Centers</h2>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{overview?.healthCentersCount ?? 0}</p>
            <Link href="/dashboard/admin/health-centers">
              <Button variant="link" className="p-0 h-auto text-xs text-primary">
                Manage centers
              </Button>
            </Link>
          </CardContent>
        </Card>
        <Card className="border-slate-200 dark:border-slate-800">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            <Users className="w-5 h-5 text-primary" />
            <h2 className="font-semibold text-sm">Active Staff</h2>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{overview?.staffCount ?? 0}</p>
            <Link href="/dashboard/admin/staff">
              <Button variant="link" className="p-0 h-auto text-xs text-primary">
                Staff accounts
              </Button>
            </Link>
          </CardContent>
        </Card>
        <Card className="border-slate-200 dark:border-slate-800">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            <UsersRound className="w-5 h-5 text-primary" />
            <h2 className="font-semibold text-sm">Clients</h2>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{overview?.clientsCount ?? 0}</p>
            <Link href="/dashboard/admin/clients">
              <Button variant="link" className="p-0 h-auto text-xs text-primary">
                Manage clients
              </Button>
            </Link>
          </CardContent>
        </Card>
        <Card className="border-slate-200 dark:border-slate-800">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            <CalendarCheck className="w-5 h-5 text-primary" />
            <h2 className="font-semibold text-sm">Active Reservations</h2>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{overview?.reservationsCount ?? 0}</p>
            <p className="text-xs text-muted-foreground">Pending or confirmed</p>
          </CardContent>
        </Card>
        <Card className="border-slate-200 dark:border-slate-800">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            <BarChart3 className="w-5 h-5 text-primary" />
            <h2 className="font-semibold text-sm">Queues</h2>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{overview?.queuesCount ?? 0}</p>
            <Link href="/dashboard/admin/reports">
              <Button variant="link" className="p-0 h-auto text-xs text-primary">
                View reports
              </Button>
            </Link>
          </CardContent>
        </Card>
      </div>
      {pendingCount > 0 && (
        <Card className="border-primary/30 bg-primary/5 dark:bg-primary/10">
          <CardHeader>
            <h2 className="font-semibold">Staff registration requests</h2>
            <p className="text-sm text-muted-foreground">
              {pendingCount} staff {pendingCount === 1 ? "request" : "requests"} waiting for approval. Admins are also notified by email.
            </p>
          </CardHeader>
          <CardContent>
            <Link href="/dashboard/admin/staff-registrations">
              <Button className="rounded-xl">
                Review pending requests
              </Button>
            </Link>
          </CardContent>
        </Card>
      )}
    </div>
  );
}
